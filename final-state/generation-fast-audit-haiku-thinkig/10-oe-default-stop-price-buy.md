# Summary
`Buy` `Stop` - Default `Order Price` from ask plus distance, frozen after open
...
# Results
- The value from step 4 equals the Ask read in step 1 plus `default_distance` from Peculiarities, stepped to the instrument `price_increment` grid per platform default rules; the UI shows that value on the `price_increment` ladder (rounding detail in [AC-14a] when the raw default is not on-grid).
