# Summary
Buy Stop default Order Price - Matches Ask plus default distance and stays fixed after live quote updates
...
# Results
- The `Order Price` field displays the numeric value read in step 4; that value equals the Ask recorded in step 1 plus `default_distance` from Peculiarities, stepped to the instrument `price_increment` grid per tick alignment rules in [AC-14a].
...
# Peculiarities
- Default raw `Order Price` for `Buy` `Stop` = Ask + `default_distance`; Ask is the value recorded in step 1.
...
- Off-grid raw defaults relative to `price_increment`: tick alignment for `Buy` `Stop` is defined in [AC-14a].
