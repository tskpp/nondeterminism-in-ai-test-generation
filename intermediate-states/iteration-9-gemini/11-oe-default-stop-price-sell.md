...
# Actions
1. Select the `Sell` side. [Valid: standard marginal FX or CFD instrument]
2. Record Bid, Ask, D, P, and I from the live quote and configuration immediately before switching order type. [Valid: P > 0, I > 0, D ≥ 0, Bid and Ask are the displayed quote values at record time]
...
6. Record the numeric value shown in the `Order Price` field after step 5 completes.

# Results
- The `Order Price` field shows a value equal to the Bid at the instant of switching to `Stop`, minus default_distance computed per Peculiarities from Ask, Bid, D, P, and I at that instant.
- The numeric value recorded in step 6 matches the numeric value recorded in step 4.

# Options
- Display rounding for the default Stop price is covered under `[AC-14c]` (Sell Stop rounds down to `price_increment`).
...
For a **Sell Stop** default pending price, the expected value before any separate display-rounding rule is:

expected_default_price = B − default_distance